...
- **記事そのものに言及しない。** intro・outro・講評のどこにも「この記事では」「本記事では」
  「本稿では」を書かない。実測で最も多く出た痕跡で、読者が量産記事だと判断する手がかりになる。
  記事の中身を予告せず、その中身を直接書き始める。
  ✗「この記事では、選び方のポイントを解説します。」
  ✓「重さで選ぶなら1kgが境目になる。ここを超えると徒歩での持ち運びが現実的でなくなる。」
- **「AIが書いた」と読者に気づかれない文章にする**（2026-09-07 追加。基準の正本は
...
- 「重要です」「大切です」と書きたくなったら、その文自体を消す。なぜそうなのかを
  spec と用途から言える事実の文で書き、書けないなら何も書かない。
- 「〜しましょう」「〜してください」と読者に呼びかける文は、記事全体で1つまで。
- ⚠️ 具体を出せないときに、数値・体験・事例を創作して埋めてはいけない。渡された
  商品データから言えないことは書かず、その箇所は短いまま終える。
...
# 商品そのものが入れ替わる記事は16本中5〜10本。残りをLLMに書き直させても講評文の内容は
# 変わらないので、その分の呼び出しを止める（picknavi と同じ手当て・同じ実装）。
